...
const Login = () => {
  ...
  const { activeProfile, setActiveProfile, availableProfiles, setAvailableGestorRoles, updateCurrentUser, login } = useSession();
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [selectedGoogleProfile, setSelectedGoogleProfile] = useState<UserProfile>('player');
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setGoogleError(null);
      try {
        const result = await authApi.googleLogin(tokenResponse.access_token, selectedGoogleProfile);
  ...
  ].filter((o) => availableProfiles.includes(o.id));

  const googleProfileOptions: Array<{ id: UserProfile; icon: typeof User }> = [
    { id: 'player', icon: User },
    { id: 'gestor', icon: Building2 },
  ];
      ...
      <div className="space-y-3">
        <div>
          ...
          <div className="flex gap-2">
            {googleProfileOptions.map(({ id, icon: Icon }) => (
              <button
                key={id}
                type="button"
                onClick={() => setSelectedGoogleProfile(id)}
          ...
          {selectedGoogleProfile === 'gestor' && (
            <p className="mt-2 text-[11px] text-muted-foreground">{t('gestorApprovalHint')}</p>
          )}
        </div>
