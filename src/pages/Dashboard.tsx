...
const Dashboard = () => {
  ...
  const preferred = CHAMPIONSHIPS.filter(c => CURRENT_USER.preferences.includes(c.sport));
  const live = preferred.filter(c => c.status === 'live');
  const upcoming = preferred.filter(c => c.status === 'upcoming');
  const others = CHAMPIONSHIPS.filter(c => !CURRENT_USER.preferences.includes(c.sport));
  ...
  return (
    <div className="space-y-10 max-w-7xl">
        ...
        <div className="flex items-end justify-between mb-2">
          <div>
            ...
            <h1 className="font-display font-black text-3xl lg:text-4xl">
              {t('hey')}, <span className="neon-text">{CURRENT_USER.name.split(' ')[0]}</span>
            </h1>
          </div>
      ...
      <Stats live={live.length} upcoming={upcoming.length} />
...
const Stats = ({ live, upcoming }: { live: number; upcoming: number }) => {
  const { t } = useLanguage();
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {[
        ...
        { label: t('mySports'), value: CURRENT_USER.preferences.length, color: 'text-neon-pink' },
